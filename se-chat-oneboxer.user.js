...
if (!localStorage.hasOwnProperty(oneboxerLocalStorageLookupKey)){
    localStorage.setItem(oneboxerLocalStorageLookupKey, JSON.stringify(defaultFormats));
}
...
oneboxer.addEventListener('click', function(){
    var storedEvents = JSON.parse(localStorage.getItem(oneboxerLocalStorageLookupKey));
    ...
    $("#_save").click(function(){
        ...
        localStorage.setItem(oneboxerLocalStorageLookupKey, JSON.stringify(storedEvents));
        $("#_close").click();
    });
...
var storedEvents = JSON.parse(localStorage.getItem(oneboxerLocalStorageLookupKey));
var formatKeys = Object.keys(defaultFormats).filter(function(format){ return storedEvents[format].on; });
function convert(){
    var storedEvents = JSON.parse(localStorage.getItem(oneboxerLocalStorageLookupKey));
    var formatKeys = Object.keys(defaultFormats).filter(function(format){ return storedEvents[format].on; }) || [];
